...
# Get the path to the root directory (one level up from python_common)
...
class YamlSettingsSource(PydanticBaseSettingsSource):
    ...
    def _load_yaml_files(self) -> dict[str, Any]:  # noqa: C901
        ...
        # Load base config
        config_path = root_dir / "python_common" / "global_config.yaml"
        try:
        ...
        # Load split YAML files from python_common/ directory
        reserved_filenames = {
            "global_config.yaml",
            "production_config.yaml",
            ".global_config.yaml",
        }
        common_dir = root_dir / "python_common"
        split_files = sorted(common_dir.glob("*.yaml"))
        for split_file in split_files:
            if split_file.name in reserved_filenames:
                continue
            # Security: skip symlinks to prevent loading files outside python_common/
        ...
        if os.getenv("DEV_ENV") == "prod":
            prod_config_path = root_dir / "python_common" / "production_config.yaml"
            if prod_config_path.exists():
                try:
                    ...
                    if prod_config_data:
                        ...
                        logger.warning(
                            "\033[33m❗️ Overwriting python_common/global_config.yaml with python_common/production_config.yaml\033[0m"
                        )
        ...
        if custom_config_path.exists():
            try:
                ...
                if custom_config_data:
                    ...
                    warning_msg = "\033[33m❗️ Overwriting default python_common/global_config.yaml with .global_config.yaml\033[0m"
